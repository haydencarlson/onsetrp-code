...
function OnLoadedLotteryEntries(player)
    ...
    for i = 1, mariadb_get_row_count() do
        ...
        else if tonumber(entry['lotto_number']) is not tonumber(generated_number) then
            AddPlayerChat(player, "You Lost the number was "..generated_number)
            end
        end
        local query = mariadb_prepare(sql, "UPDATE status = 'expired'; FROM lotteries WHERE status = 'open';")
          mariadb_query(sql, query)
    end
end