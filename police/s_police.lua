...
function DespawnPoliceCar(player)
    ...
    if PlayerData[player].job_vehicle ~= nil then
        ...
        DestroyVehicleData(PlayerData[player].job_vehicle)
        PlayerData[player].job_vehicle = nil
        CallRemoteEvent(player, 'KNotify:Send', _("vehicle_stored"), "#0f0")
        return
    end
end
...
function CuffPlayer(player)
    ...
    if target ~= nil and PlayerData[target].is_cuffed == 0 then
        if GetNumberOfItem(player, "handcuffs") > 0 then
            ...
            CallRemoteEvent(player, 'KNotify:Send', _("player_is_handcuffed"), "#0f0")
            CallRemoteEvent(target, 'KNotify:Send', _("you_are_handcuffed"), "#f00")
        else
            CallRemoteEvent(player, 'KNotify:Send', _("no_handcuffs"), "#f00")
        end
    elseif target ~= nil and PlayerData[target].is_cuffed == 1 then
        SetPlayerCuffed(target, false)
        AddInventory(player, "handcuffs", 1)
        CallRemoteEvent(player, 'KNotify:Send', _("player_is_uncuffed"), "#0f0")
        CallRemoteEvent(target, 'KNotify:Send', _("you_are_uncuffed"), "#0f0")
    end
end
...
function AddFineInDb(player, target, amount, reason)
    ...
    -- Notify
    CallRemoteEvent(player, 'KNotify:Send', _("fine_given", amount), "#0f0")
    CallRemoteEvent(target, 'KNotify:Send', _("fine_received", amount), "#f00")
end
...
function SetPlayerInCar(player, target)
    ...
    if GetDistance3D(x, y, z, x2, y2, z2) <= 400 then
        if GetVehiclePassenger(PlayerData[player].job_vehicle, 3) == 0 then -- First back seat
            ...
            CallRemoteEvent(player, 'KNotify:Send', _("policecar_place_player_in_back"), "#0f0")
        elseif GetVehiclePassenger(PlayerData[player].job_vehicle, 4) == 0 then -- Second back seat
            ...
            CallRemoteEvent(player, 'KNotify:Send', _("policecar_no_more_seat"), "#f00")
        end
    else -- Too far away
        CallRemoteEvent(player, 'KNotify:Send', _("policecar_too_far_away"), "#f00")
    end
end
...
function RemovePlayerInCar(player)
    ...
    if GetDistance3D(x, y, z, x2, y2, z2) <= 200 then
        ...
        CallRemoteEvent(player, 'KNotify:Send', _("policecar_player_remove_from_car"), "#0f0")
    end
end
