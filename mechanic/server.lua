...
function ApplyNos(player, vehicle) 
    ...
    if GetPlayerCash(player) >= cost then
        ...
        local update_query = mariadb_prepare(sql, "UPDATE player_garage set nos_equipped = 1 WHERE id = '?';", vehicle)
...
function ApplyColorChange(player, vehicle, color_name)
    ...
    if GetPlayerCash(player) >= cost then
        ...
        local update_query = mariadb_prepare(sql, "UPDATE player_garage set color = '?' WHERE id = '?';", vehicleColors[color_name], vehicle)
        mariadb_query(sql, update_query)
        StartVehicleEngine(vehicle)
